...
void print_irq_info(int i)
{
	...
	if (zte_irq_desc) {
		...
		if (wake_prop > 0) {
			pr_info("[IRQ] num=%d, chipName=(%10s), actionName=%s, wake_depth=%d", i,
				zte_irq_desc->irq_data.chip->name ? : "-", action->name, wake_prop);
		} else {
			pr_info("[IRQ] num=%d, chipName=(%10s), actionName=%s", i,
				zte_irq_desc->irq_data.chip->name ? : "-", action->name);
		}
