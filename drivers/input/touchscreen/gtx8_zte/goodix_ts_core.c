...
static struct workqueue_struct *delay_work_workqueue;

static struct goodix_probe_struct
{
	struct work_struct delay_probe_work;
	struct platform_device *pdev;
} goodix_work_struct_probe;
...
{
	struct goodix_ts_device *ts_device = goodix_modules.core_data->ts_dev;
 ...
 */
static void goodix_ts_probe(struct work_struct *work)
{
	struct goodix_ts_core *core_data = NULL;
	struct goodix_ts_device *ts_device;
	int r;
	struct goodix_probe_struct *gp =
			container_of(work, struct goodix_probe_struct, delay_probe_work);

	struct platform_device *pdev = gp->pdev;
...
static int goodix_ts_workqueue_probe(struct platform_device *pdev)
{
	...
	delay_work_workqueue = create_singlethread_workqueue("goodix_work_probe");
	if (!delay_work_workqueue) {
		pr_err("%s: create delay_probe_workqueue faile!\n", __func__);
		retval = -ENOMEM;
		goto exit;
	}

	INIT_WORK(&(goodix_work_struct_probe.delay_probe_work), goodix_ts_probe);

	goodix_work_struct_probe.pdev = pdev;

	queue_work(delay_work_workqueue,
				&(goodix_work_struct_probe.delay_probe_work));
	retval = 0;
...
static int goodix_ts_remove(struct platform_device *pdev)
{
	...
	cancel_work_sync(&(goodix_work_struct_probe.delay_probe_work));
	flush_workqueue(delay_work_workqueue);
	destroy_workqueue(delay_work_workqueue);
