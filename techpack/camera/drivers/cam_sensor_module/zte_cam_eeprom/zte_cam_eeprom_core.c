...
/* use default eeprom data start*/
#include <linux/firmware.h>
/* use default eeprom data end*/
...
static void use_default_eeprom_setting(struct cam_eeprom_ctrl_t *e_ctrl, uint8_t *filename)
{
	const struct firmware *fw = NULL;
	uint8_t fw_state = 0;

	CAM_ERR(CAM_EEPROM, "EEPROM check fail, use default setting file:%s\n", filename);

	if (!e_ctrl->soc_info.dev)
		CAM_ERR(CAM_EEPROM, "Error,No eeprom dev\n");
	else {
		fw_state = request_firmware(&fw, filename, e_ctrl->soc_info.dev);
		if (fw_state) {
			CAM_ERR(CAM_EEPROM, "Error,failed to request firmware: %s\n", filename);
		} else {
			if (e_ctrl->cal_data.num_data != fw->size)
				CAM_ERR(CAM_EEPROM, "Error,default setting file is not correct\n");
			else
				memcpy(&(e_ctrl->cal_data.mapdata[0]), (uint8_t *)fw->data, e_ctrl->cal_data.num_data);
			release_firmware(fw);
		}
	}
}

static int cam_eeprom_check_memory(struct cam_eeprom_ctrl_t *e_ctrl)
{
	int	rc = 0;
	...
	if (e_ctrl->checksum) {
		if (e_ctrl->eeprom_fun_p->get_default_eeprom_setting_filename) {
			use_default_eeprom_setting(e_ctrl,
				e_ctrl->eeprom_fun_p->get_default_eeprom_setting_filename(e_ctrl));
			if (e_ctrl->eeprom_fun_p->eeprom_checksum)
				e_ctrl->checksum = e_ctrl->eeprom_fun_p->eeprom_checksum(e_ctrl);
		}
	}
	...
	if (e_ctrl->eeprom_fun_p->set_cali_info_valid) {
		e_ctrl->eeprom_fun_p->set_cali_info_valid(e_ctrl);
	}
...
/**
 * zte_cam_eeprom_parse_read_memory_map - Parse memory map
 * @of_node:    device node
 * @e_ctrl:     ctrl structure
 *
 * Returns success or failure
	...
	struct cam_eeprom_ctrl_t *e_ctrl)
{
	...
	rc = zte_cam_eeprom_parse_dt_memory_map(of_node, &e_ctrl->cal_data);
	if (rc) {
	...
	if (e_ctrl->eeprom_fun_p->read_eeprom_memory) {
		rc = e_ctrl->eeprom_fun_p->read_eeprom_memory(e_ctrl, &e_ctrl->cal_data);
	} else {
	...
	rc = cam_eeprom_check_memory(e_ctrl);

	if (rc) {
		CAM_ERR(CAM_EEPROM,
			"cam_eeprom_check_memory failed");
		goto power_down;
	}
	...
	for (i = 0; i < csl_packet->num_io_configs; i++) {
		...
		if (io_cfg->direction == CAM_BUF_OUTPUT) {
			...
			for (j = 0; j < e_ctrl->cal_data.num_data; j++)
				CAM_DBG(CAM_EEPROM, "0x%4x 0x%2x", j, read_buffer[j]);
		} else {
...
static int32_t cam_eeprom_pkt_parse(struct cam_eeprom_ctrl_t *e_ctrl, void *arg)
{
	...
	case CAM_EEPROM_PACKET_OPCODE_INIT:
		if (e_ctrl->probed == true) {
			rc = cam_eeprom_get_cal_data(e_ctrl, csl_packet);
			return rc;
		}
		if (e_ctrl->userspace_probe == false) {
			rc = zte_cam_eeprom_parse_read_memory_map(
...
/**
 * zte_cam_eeprom_driver_cmd - Handle eeprom cmds
 * @e_ctrl:     ctrl structure
 * @arg:        Camera control command argument
 *
 * Returns success or failure
